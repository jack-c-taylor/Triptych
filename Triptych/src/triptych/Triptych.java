...
public class Triptych extends JFrame implements ActionListener{
	...
	private JTextArea history=new JTextArea(10,20);
	private JProgressBar player1=new JProgressBar(0,2000);
	private JProgressBar player2=new JProgressBar(0,2000);
	...
	public void newCardField(){
		...
		field.add(Box.createRigidArea(new Dimension(300,30)));
		field.add(handField);
	}
	...
	public void newCardDescriptionField(){
		...
		descriptionField.add(Box.createRigidArea(new Dimension(300,5)));
		descriptionField.add(description);
	}
	
	/**
	 * Generates the text fields for information on the players and adds borders between them.
	 */
	public void newStatusField(){
		//Generates the text fields and adds borders between them.
		statusField.setLayout(new BoxLayout(statusField, EXIT_ON_CLOSE));
		statusField.add(new JLabel("Player 1 health: "));
		statusField.add(Box.createRigidArea(new Dimension(0,5)));
		statusField.add(player1);
		statusField.add(Box.createRigidArea(new Dimension(0,5)));
		statusField.add(new JLabel("Player 2 health: "));
		statusField.add(Box.createRigidArea(new Dimension(0,5)));
		statusField.add(player2);
		statusField.add(Box.createRigidArea(new Dimension(0,5)));
		statusField.add(new JLabel("Last moves: "));
		statusField.add(Box.createRigidArea(new Dimension(0,5)));
		statusField.add(history);
		statusField.add(Box.createRigidArea(new Dimension(150,5)));
		player1.setValue(gameProperties.getPlayer1().getHealth());
		player2.setValue(gameProperties.getPlayer2().getHealth());
